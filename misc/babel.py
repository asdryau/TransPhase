...
def get_split(path: str, split: str, subset: Optional[str] = ''):
    ...
    filepath = Path(path) / f'../babel-teach/babel-smplh-30fps-male/{split}{subset}.pth.tar'
...
class BABEL(Dataset):
    ...
    def __init__(self, datapath: str,
                 split: str = "train",
                 sampler=None,
                 framerate: float = 30.0,   # it should already be 30
        ...
        for i, sample in enumerator:
            ...
            for index, seg in enumerate(seg_ids):
                ...
                if not self.dtype == 'separate_pairs':
                    ...
                    if not self.sampler.accept(dur1) or not self.sampler.accept(dur2+dur_tr) or (dur_tr*2 > dur1 + dur2):
                ...
                print(f"{babel_id}-{index}", smpl_data["poses"].shape, duration, seg_acts[index])
                # exit()
                ...
                # if mode == 'train':
                motion_data[f'{babel_id}-{index}'] = smpl_data
                # else:
        ...
        ### save motion into pkl
        save_pkl = [motion_data, texts_data, durations]
        with open(f'./babel-teach-{split}.pkl', 'wb') as handle:
...
sampler = FrameSampler(min_len=45, max_len=250)
hparam = {"split": "train", "tiny": False, "progress_bar": True, "datapath":"", "sampler": sampler}
BABEL(**hparam)